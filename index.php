<?php
/**
 * Mikeotizels DirLister plugin. 
 * 
 * A simple PHP program that lists files and folders in a directory.
 * 
 * @link https://github.com/mikeotizels/dirlister/
 * 
 * @author    Michael Otieno <mikeotizels@gmail.com>
 * @copyright Copyright 2020-2024 Michael Otieno
 * @license   The MIT License (https://opensource.org/licenses/MIT)
 * @version   2.0.0
 */

// Set PHP error reporting
// - Set to -1 or E_ALL to enable error reporting (for development environment)
// - Set to 0 or 'none' to disable error reporting (for production environment)  
@error_reporting(0);

// Set the PHP INI directive to restrict all file operations within the server
// document root.  
@ini_set('open_basedir', $_SERVER['DOCUMENT_ROOT']);

###############################################################################
## Begin - DirLister Options 
##
 
/** 
 * The color scheme to use for the directory listing page.
 * 
 * Possible values are either 'light' or 'dark'. The default value is 'light'. 
 * 
 * @since 1.0.0
 * 
 * @deprecated 2.0.0 The theme is changed dynamically using a theme switch in
 *                   the page header.
 * 
 * @global string
 */
$theme = 'light';

/**
 * The meta title of the document and also the title of the page.
 * 
 * If blank, title will be set based on the name of the directory where the 
 * DirLister's index.php file is placed.
 * 
 * @since 1.0.0
 * 
 * @global string
 */ 
$title = '';

/**
 * Full path to the directory to be scanned for listing.
 * 
 * Must be relative to the document root and must be readable by the server.
 * 
 * The directory where the DirLister's index.php file is placed will be opened 
 * and its contents read if not specified here.
 * 
 * @since 2.0.0
 * 
 * @global string
 */
$dir = ''; # /path/to/your/directory
  
/**
 * How to sort the folders and files.
 * 
 * Possible values are: 
 * 
 *   'NA' - Sort by file/folder name (ascending)
 *   'ND' - Sort by file/folder name (descending)
 *   'DA' - Sort by date created/modified (ascending)
 *   'DD' - Sort by date created/modified (descending) 
 * 
 * Default is to sort the folders and files by their names in ascending order. 
 * 
 * @since 1.0.0
 * 
 * @global string
 */
$sort = 'NA'; 

/**
 * List of files to exclude when displaying the directory contents. 
 * 
 * File entry is a shell-style wildcard expression or full filename. 
 * 
 * By default, the plugin skips the current (.) and parent folder (..) links 
 * and ignores "hidden" folders and files starting with a period.
 * 
 * Default: 
 * 
 * [
 *    '.', 
 *    '..', 
 *    '.??*', 
 *    '*~', 
 *    '*#', 
 *    'HEADER*', 
 *    'README*', 
 *    'RCS', 
 *    'CVS', 
 *    '*,v', 
 *    '*,t',
 *    '*.bak', 
 *    '*.log'
 * ]
 * 
 * You can add specific file names and extensions you want to hide or remove 
 * specific file names and extensions you want to get listed. 
 * 
 * @since 2.0.0
 * 
 * @global array 
 */ 
$exclude = [
    '.', 
    '..', 
    '.??*', 
    '*~', 
    '*#', 
    'HEADER*', 
    'README*', 
    'RCS', 
    'CVS',
    'error_log', 
    '*,v', 
    '*,t',
    '*.db',  
    '*.ini', 
    '*.log',
    '*.tmp'
];
 
/**
 * A set of filenames which the directory indexing script should ignore and not 
 * include in the listing.
 * 
 * @since 1.0.0
 * 
 * @deprecated 2.0.0 
 * @see $exclude 
 * 
 * @global array
 */
$exclude_filenames = [];
   
/**
 * A set of file extensions which the directory indexing script should ignore 
 * and not include in the listing.
 * 
 * @since 1.0.0 
 * 
 * @deprecated 2.0.0 
 * @see $exclude 
 * 
 * @global array
 */
$exclude_extensions = [];

/** 
 * Whether to hide empty folders. Default is true.
 * 
 * You can turn this off by changing the value to false.
 * 
 * @since 1.0.0
 * 
 * @global boolean
 */
$ignore_empty_folders = true;

/** 
 * Whether to toggle sub folders. Default is true.
 * 
 * If true, the sub-folders will be hidden by default and when the parent 
 * folder is clicked on, it will slide down the sub-folders.
 * 
 * @since 1.0.0
 * 
 * @global boolean
 */
$toggle_sub_folders = true;
  
/**
 * Whether to enable the force download attribute. Default is true.
 * 
 * You can set this to true to add the HTML download attribute which forces  
 * download in some browsers.
 * 
 * @since 1.0.0 
 * 
 * @global boolean
 */
$force_download = true;

/** 
 * Whether to show the file modification date (time when the content of the 
 * file was last changed). Default is false.
 * 
 * @since 2.0.0
 * 
 * @global boolean
 */
$show_filemtime = false;

##
## End - DirLister Options 
###############################################################################

###############################################################################
## Begin - DirLister Functions
##

/**
 * Lists files and folders in a directory.
 * 
 * @since 2.0.0
 * 
 * @param string $path     Path to the directory.
 * @param array  $excludes List of files to exclude in the directory list.
 *                         File is a shell-style wildcard expression or full 
 *                         filename. The current (.) and parent folder (..) 
 *                         links and "hidden" folders and files starting with 
 *                         a period are ignored by the default.
 * 
 * @return array|false Array of files. False if unable to list contents.
 */
function dir_list($path, &$excludes) {
    if (empty($path)) {
        $path = __DIR__;
    }

    if (!is_dir($path)) {
        return false;
    }

    if (!is_readable($path)) {
        return false;
    }

    $path = normalize_path($path);
    $dirh = opendir($path);

    if ($dirh === false) {
        return false;
    }
        
    // Add this "index.php" file to the exclusions list.
    $excludes = array_merge(['index.php'], $excludes);
    $files    = [];

    while (($file = readdir($dirh)) !== false) {
        // Skip current and parent folder links.
        if (in_array($file, ['.', '..'], true)) {
            continue;
        }

        // Skip "hidden" folders and files starting with a period.
        if ($file[0] === '.'|| $file[0] === '..') {
            continue;
        }

        // Check if a filename matches the ignore shell wildcard pattern.
        if (should_exclude($file, $excludes)) {
            continue;
        }

        $files[] = $file;
    }
    
    closedir($dirh);

    return $files;
}

/**
 * Shows the file block.
 * 
 * Original code comes from DirectoryLister v1.11
 *   (https://github.com/halgatewood/file-directory-list/)
 * Written by Hal Gatewood in 2015, 
 *   (https://halgatewood.com/free/file-directory-list/)
 * and license under the MIT License 
 *   (https://opensource.org/licenses/MIT)
 * 
 * @since 1.0.0
 * 
 * @param string $file
 * 
 * @return void
 */ 
function display_block($file) {
    global $dir, $force_download, $show_filemtime;

    if (empty($dir)) {
        $dir = __DIR__;
    }

    // TODO: Use function get_file_dir() and get_file_url()
    $doc_root   = normalize_path($_SERVER['DOCUMENT_ROOT']);
    $base_dir   = normalize_path($dir);
    $file_dir   = $base_dir . '/' . $file;
    $file_url   = str_replace($doc_root, '', $base_dir) . '/' . $file;
    $file_name  = basename($file_dir);
    $file_ext   = get_file_ext($file_dir);
    $file_size  = format_file_size(filesize($file_dir));
    $file_mtime = "";

    if (is_dir($file_dir)) {
        $file_ext = 'dir';
    }

    $download_attr = ($force_download AND $file_ext != 'dir') ? "download=\"$file_name\"" : "";

    if ($show_filemtime) {
        $file_mtime = date('d/m/Y h:i A', filemtime($file_dir));
    }
  
    $rtn = "<div class=\"block\">\n";

    if ($file_ext === 'dir') {
        $rtn .= "<a class=\"dir\" href=\"$file_url\" title=\"Open\">\n";
    } else {
        $rtn .= "<a class=\"file\" href=\"$file_url\" {$download_attr} title=\"Download\">\n";
    }

    $rtn .= "<div class=\"img $file_ext\"></div>\n";
    $rtn .= "<div class=\"meta\">\n";
  
    if ($file_ext === 'dir') {
        $rtn .= "<div class=\"name\">$file_name</div>\n";

        $dir_fcnt = count_dir_files($file_dir);
        $dir_size = format_file_size(get_dir_size($file_dir));

        if ($dir_fcnt == 1) {
            $rtn .= "<div class=\"info\">1 File";
        } else {
            $rtn .= "<div class=\"info\">$dir_fcnt Files";
        }

        $rtn .= " (<b>$dir_size</b>)</div>\n";
    } else {
        $rtn .= "<div class=\"name\">$file_name</div>\n";
        $rtn .= "<div class=\"size\"><b>Size:</b> $file_size</div>\n";

        if (!empty($file_mtime)) {
            $rtn .= "<div class=\"modified\"><b>Date Modified:</b> $file_mtime</div>\n";
        }
    }

    $rtn .= "</div>\n";
    $rtn .= "</a>\n";
    $rtn .= "</div>\n";

    return $rtn;
}

/**
 * Builds the blocks recursively.
 * 
 * Original code comes from DirectoryLister v1.11
 *   (https://github.com/halgatewood/file-directory-list/)
 * Written by Hal Gatewood in 2015, 
 *   (https://halgatewood.com/free/file-directory-list/)
 * and license under the MIT License 
 *   (https://opensource.org/licenses/MIT)
 * 
 * @since 1.0.0
 * 
 * @param array       $items
 * @param string|null $folder
 * 
 * @return void
 */
function build_blocks($items = [], $folder = null) { 
    global $dir, $sort, $exclude, $ignore_empty_folders, $toggle_sub_folders;

    if (empty($items)) {
        $items = dir_list($dir, $exclude);
    }  
      
    $objects            = [];
    $objects['folders'] = [];
    $objects['files']   = [];

    foreach($items as $item) {
        if ($folder && $item) {
            $item = $folder . '/' . $item;
        }
  
        // Folders
        if (!empty($dir)) {
            if (is_dir($dir . '/' . $item)) {
                $objects['folders'][] = $item; 
                continue;
            }
        } else {
            if (is_dir($item)) {
                $objects['folders'][] = $item; 
                continue;
            }
        }
  
        // Files
        if ($item) {
            $objects['files'][] = $item;
        }
    }
  
    foreach ($objects['folders'] as $file) {
        $sub_items = [];

        if (!empty($dir)) {
            $sub_items = dir_list($dir  . '/' . $file, $exclude);
        } else {
            $sub_items = dir_list($file, $exclude);
        }

        if ($ignore_empty_folders) {
            $has_sub_items = false;

            foreach ($sub_items as $sub_item) {
                if ($sub_item == '.' OR $sub_item == '..') {
                    continue;
                }

                $has_sub_items = true;
        
                break;  
            }

            if ($has_sub_items) { 
                echo display_block($file);
            }
        } else {
            echo display_block($file);
        }
    
        if ($sub_items) {
            if ($toggle_sub_folders) {
                // TODO: Use function get_file_url()
                $doc_root = normalize_path($_SERVER['DOCUMENT_ROOT']);
                $base_dir = normalize_path($dir);
                $file_url = str_replace($doc_root, '', $base_dir) . '/' . $file;

                echo "<div class=\"sub\" data-parent=\"$file_url\">";
                build_blocks($sub_items, $file);
                echo "</div>";
            } else {
                echo "<div class=\"sub show\">";
                build_blocks($sub_items, $file);
                echo "</div>";                
            }
        }
    }

    // Sort before loop
    switch ($sort) {
        case 'DA':
            ksort($objects['files']);  
            break;

        case 'DD':
            krsort($objects['files']);  
            break;
            
        case 'ND':
            arsort($objects['files']);
            break;
        
        case 'NA';
        default:
            natsort($objects['files']);
            break;
    }

    foreach($objects['files'] as $file) {
        echo display_block($file);
    }
}

// ----------------------------------------------------------------------------
// Utility Functions
// ----------------------------------------------------------------------------

/**
 * Normalizes a directory path.
 * 
 * @since 2.0.0
 * 
 * @param string $path
 * 
 * @return string
 */
function normalize_path($path) {
    // Strip out Windows drive letter if it's there.
    $path = preg_replace('|^([a-z]{1}):|i', '', $path);  
    // Windows path sanitisation.
    $path = str_replace('\\', '/', $path);

    // TODO: Remove slash at the end of the path?
    return $path;
}

/**
 * Formats a directory or file size in the appropriate file measurement unit. 
 *
 * @since 1.0.0
 *
 * @param integer $bytes     Required. Total number of bytes.
 * @param integer $precision Optional. Precision of decimal places. Default 2.
 * 
 * @return string|false The formatted size string on success, otherwise false.
 */
function format_file_size($bytes, $precision = 2) {
    $units = ['B', 'KB', 'MB', 'GB', 'TB'];
    $bytes = max($bytes, 0); 
    $pow   = floor(($bytes ? log($bytes) : 0) / log(1024)); 
    $pow   = min($pow, count($units) - 1); 
    $bytes /= (1 << (10 * $pow));

    return round($bytes, $precision) . ' ' . $units[$pow];
}

/**
 * Gets the extension of a file.
 * 
 * @since 1.0.0
 * 
 * @param string $file
 * 
 * @return string
 */
function get_file_ext($file) {
    return substr(strrchr($file, '.'), 1);
}

/**
 * Gets the size of a directory in bytes.
 * 
 * @since 1.0.0
 * 
 * @param string $path
 * 
 * @return integer
 */
function get_dir_size($path) {
    if (empty($path)) {
        return false;
    }

    $bytes = 0;
    $path  = realpath($path);

    if ($path !== false && is_dir($path)) {
        foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($path, FilesystemIterator::SKIP_DOTS)) as $object) {
            $bytes += $object->getSize();
        }
    }
    
    return $bytes;
}

/**
 * Counts the number of files in a directory.
 * 
 * @since 1.0.0
 * 
 * @param string $path
 * 
 * @return integer
 */
function count_dir_files($path) {
    $fsi = new FilesystemIterator($path, FilesystemIterator::SKIP_DOTS);
    return iterator_count($fsi);
}

/**
 * Checks if a file matches the ignore shell wildcard pattern.
 * 
 * @since 2.0.0
 * 
 * @param string $file
 * @param array  $excludes
 * 
 * @return boolean
 */
function should_exclude($file, &$excludes) {
    $exclude = reset($excludes);

    foreach($excludes as $exclude) {
        if (fnmatch($exclude, $file)) {
            return true;
        }
    }

    return false;
}

/**
 * Gets a page title from the directory name.
 * 
 * Capitalizes the output and replaces any underscores and dashes in the path 
 * with whitespaces.
 * 
 * @since 2.0.0
 * 
 * @param string The absolute directory path. Defaults to the of directory name 
 *               of the current file if empty.
 * 
 * @return string The title from the file directory name.
 */
function get_page_title($path) {
    return ucwords(str_replace(['-', '_'], ' ', basename($path)));
}

##
## End - DirLister Functions
###############################################################################

// Set the page title based on folder name, if not set above.
if (empty($title)) { 
    $title = get_page_title(dirname(__FILE__));
}
?>
<!DOCTYPE html>
<html lang="en-US" theme="light">
<head>        
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1"> 
    <meta name="referrer" content="strict-origin-when-cross-origin">  
    <meta name="robots" content="noindex, nofollow">
    <meta name="author" content="Michael Otieno">
    <title><?=$title?></title>
    <style type="text/css">
    :root,[theme="light"]{color-scheme:light;--body-color:#212529;--body-background-color:#f8f9fa;--border-color:#dee2e6;--switch-icon-image:url('data:image/svg+xml,%3csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="%231f1f1f"%3e%3cpath d="M256 160c-52.9 0-96 43.1-96 96s43.1 96 96 96 96-43.1 96-96-43.1-96-96-96zm246.4 80.5l-94.7-47.3 33.5-100.4c4.5-13.6-8.4-26.5-21.9-21.9l-100.4 33.5-47.4-94.8c-6.4-12.8-24.6-12.8-31 0l-47.3 94.7L92.7 70.8c-13.6-4.5-26.5 8.4-21.9 21.9l33.5 100.4-94.7 47.4c-12.8 6.4-12.8 24.6 0 31l94.7 47.3-33.5 100.5c-4.5 13.6 8.4 26.5 21.9 21.9l100.4-33.5 47.3 94.7c6.4 12.8 24.6 12.8 31 0l47.3-94.7 100.4 33.5c13.6 4.5 26.5-8.4 21.9-21.9l-33.5-100.4 94.7-47.3c13-6.5 13-24.7.2-31.1zm-155.9 106c-49.9 49.9-131.1 49.9-181 0-49.9-49.9-49.9-131.1 0-181 49.9-49.9 131.1-49.9 181 0 49.9 49.9 49.9 131.1 0 181z"/%3e%3c/svg%3e');} [theme="dark"]{color-scheme:dark;--body-color:#dee2e6;--body-background-color:#212529;--border-color:#495057;--switch-icon-image:url('data:image/svg+xml,%3csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="%23ffffff"%3e%3cpath d="M283.211 512c78.962 0 151.079-35.925 198.857-94.792 7.068-8.708-.639-21.43-11.562-19.35-124.203 23.654-238.262-71.576-238.262-196.954 0-72.222 38.662-138.635 101.498-174.394 9.686-5.512 7.25-20.197-3.756-22.23A258.156 258.156 0 0 0 283.211 0c-141.309 0-256 114.511-256 256 0 141.309 114.511 256 256 256z"/%3e%3c/svg%3e');} *,*::before,*::after{-webkit-box-sizing:border-box;-moz-box-sizing:border-box;box-sizing:border-box;} body{text-align:left;font-family:"Segoe UI","Verdana","Arial",sans-serif;font-size:1rem;font-weight:400;line-height:1.5;color:var(--body-color);background-color:var(--body-background-color);margin:0;overflow-x:hidden;-webkit-text-size-adjust:100%;-webkit-tap-highlight-color:rgba(0,0,0,0);} noscript{position:fixed;top:0;left:0;right:0;z-index:999999999 !important;color:#721c24;background-color:#f8d7da;border:1px solid transparent;border-bottom:1px solid #dc3545;font-size:0.875rem;font-weight:400;line-height:1.5;text-decoration:none;margin:0;padding:0.75rem 1.25rem;width:auto;height:auto;word-wrap:break-word;overflow:hidden;} .container{padding-right:1.5rem;padding-left:1.5rem;margin-right:auto;margin-left:auto;width:100%;} .header{margin-top:1.5rem;margin-bottom:1.5rem;} .header-wrapper{width:100%;display:flex;align-items:center;} .header-title{width:60%;text-align:left;} .header-switch{width:40%;text-align:right;} .title-heading{color:inherit;font-size:calc(1.375rem + 1.5vw);font-weight:300;line-height:1.2;margin-top:0;margin-bottom:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;} .switch-label{border:1px solid;border-radius:1.875rem;padding:0.375rem 0.75rem;cursor:pointer;} .switch-check{display:none;} .switch-icon{display:inline-block;background-image:var(--switch-icon-image);width:1.5rem;height:1.5rem;vertical-align:-0.4em;} .switch-icon-{} .main{min-height:100vh;} .content{background-color:#ffffff;background-clip:border-box;border:1px solid var(--border-color);border-radius:0.5rem;padding:1.5rem;} .block{clear:both;overflow:hidden;border-top:solid 1px #ece9e9;min-height:3rem;} .block:first-child{border-top:0} .block a{display:block;color:#1f1f1f;padding:1rem;text-decoration:none;-webkit-transition:all 0.25s ease-in-out;-moz-transition:all 0.25s ease-in-out;-o-transition:all 0.25s ease-in-out;transition:all 0.25s ease-in-out;} .block a:hover,.block a:focus{color:#0d63c9;text-decoration:none;} .block a.dir:hover,.block a.dir:focus{background-color:#bfbfbf} .block a.file:hover,.block a.file:focus{background-color:#f2f2f2;} .sub{display:none;border-left:solid 5px #bfbfbf;margin-left:20px;} .sub.show{display:block;} .sub a.dir:hover,.sub a.dir:focus{background-color:#d9d9d9} .sub a.file:hover,.sub a.file:focus{background-color:#f2f2f2;} .img{display:block;float:left;background:url('data:image/png;base64,') left top no-repeat;margin-right:0.5rem;height:3rem;width:3rem;vertical-align:middle;} .img.gif,.img.jpeg,.img.jpg,.img.png{background-position:-50px 0 !important} .img.pdf{background-position:-100px 0 !important} .img.rtf,.img.txt{background-position:-150px 0 !important} .img.xls,.img.xlsx{background-position:-200px 0 !important} .img.ppt,.img.pptx{background-position:-250px 0 !important} .img.doc,.img.docx{background-position:-300px 0 !important} .img.gzip,.img.rar,.img.tar,.img.zip{background-position:-350px 0 !important} .img.swf{background-position:-400px 0 !important} .img.fla{background-position:-450px 0 !important} .img.mp3{background-position:-500px 0 !important} .img.wav{background-position:-550px 0 !important} .img.mp4{background-position:-600px 0 !important} .img.aiff,.img.avi,.img.m2v,.img.mov,.img.pict,.img.qif{background-position:-650px 0 !important} .img.avi,.img.mpg,.img.wmv{background-position:-700px 0 !important} .img.flv,.img.f2v{background-position:-750px 0 !important} .img.psd{background-position:-800px 0 !important} .img.ai{background-position:-850px 0 !important} .img.asp,.img.css,.img.dhtml,.img.html,.img.inc,.img.js,.img.php,.img.xhtml{background-position:-900px 0 !important} .img.dir{background-position:-950px 0 !important} .meta .name{font-weight:bold;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;} .meta .info,.meta .size,.meta .modified{color:#6c757d;font-size:0.875rem;line-height:1.2} .footer{text-align:center;border-top:1px solid var(--border-color);} .footer-signature{font-size:0.875rem;} .footer-link{color:inherit;text-decoration:none;} .footer-link:hover{text-decoration:underline;} @media (min-width:576px){.container{max-width:540px}} @media (min-width:768px){.container{max-width:720px}} @media (min-width:992px){.container{max-width:960px}} @media (min-width:1200px){.container{max-width:1140px} .title-heading{font-size:2.5rem}} @media (min-width:1400px){.container{max-width:1320px}} @media (max-width:575.98px){} @media (max-width:767.98px){} @media (max-width:991.98px){}
    </style>
</head>
<body>
    <header id="header" class="header">
        <div class="container">
            <div class="header-wrapper">
                <div class="header-title">
                    <h1 class="title-heading"><?=$title?></h1>
                </div><!-- .header-title -->
                <div class="header-switch">
                    <label id="themeSwitchLabel" for="themeSwitchCheck" class="switch-label" title="Toggle theme">
                        <input type="checkbox" id="themeSwitchCheck" class="switch-check">
                        <span id="themeSwitchIcon" class="switch-icon"></span>
                    </label>
                </div><!-- .header-switch -->               
            </div><!-- .header-wrapper -->
        </div><!-- .container -->     
    </header><!-- #header -->
    <main id="main" class="main">
        <div class="container">
            <div class="content">
                <?php build_blocks(); ?>
            </div><!-- .content -->           
        </div><!-- .container -->       
    </main><!-- #main -->
    <br><br>
    <footer id="footer" class="footer">
        <div class="container"> 
            <p class="footer-signature">
                <a class="footer-link" href="https://github.com/mikeotizels/dirlister/" target="_blank" rel="noopener noreferrer nofollow">Proudly Served by DirLister</a>
            </p>
        </div><!-- .container -->     
    </footer><!-- #footer -->
    <noscript>
        This page functions properly with JavaScript enabled. 
    </noscript> 
    <script type="text/javascript">
    !function(o,i){o.addEventListener("load",function(){var e,t;e=localStorage.getItem("theme")||(o.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"),t=i.documentElement.getAttribute("theme"),e&&e!==t&&i.documentElement.setAttribute("theme",e)}),i.addEventListener("DOMContentLoaded",function(){(function(){var e=localStorage.getItem("theme");const t=i.getElementById("themeSwitchLabel"),n=i.getElementById("themeSwitchCheck");t&&n&&(e&&(i.documentElement.setAttribute("theme",e),"dark"===e?(n.checked=!0,t.setAttribute("title","Toggle light mode")):(n.checked=!1,t.setAttribute("title","Toggle dark mode"))),n.addEventListener("change",function(){this.checked?(o.localStorage.setItem("theme","dark"),i.documentElement.setAttribute("theme","dark"),t.setAttribute("title","Toggle light mode")):(o.localStorage.setItem("theme","light"),i.documentElement.setAttribute("theme","light"),t.setAttribute("title","Toggle dark mode"))}))})(),i.querySelectorAll("a.dir").forEach(function(e){e.addEventListener("click",function(e){e.preventDefault(),this.classList.toggle("open"),i.querySelector('.sub[data-parent="'+this.getAttribute("href")+'"]').classList.toggle("show")})})})}(window,document);
    </script>
</body>
</html>
