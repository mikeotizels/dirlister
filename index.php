<?php
/**
 * Mikeotizels DirLister
 * 
 * This program lists files and folders in a directory.
 * 
 * @author    Michael Otieno <mikeotizels@gmail.com>
 * @copyright Copyright 2020 Michael Otieno
 * @license   The MIT License (https://opensource.org/licenses/MIT)
 * @version   1.0.0
 */

// Set PHP error reporting
// - Set to -1 or E_ALL to enable error reporting (for development environment)
// - Set to 0 or 'none' to disable error reporting (production environment)  
error_reporting(0);

###############################################################################
## Begin - DirLister Options 
##
  
// Theme color for the look and feel of generated index listing.
// - Change this to "dark" when you are feeling down.
$theme = "light";

// The meta title of the document and also the title of the page.
// - The title will be set based on current folder name if left blank here.
$title = "";
  
// How to sort the folders and files.
// Possible values are: 
//   'NA' - File/folder name (ascending)
//   'ND' - File/folder name (descending)
//   'DA' - Date created/modified (ascending)
//   'DD' - Date created/modified (descending) 
// Default is to sort the folders and files by their names in ascending order. 
$sort_by = "NA"; 

// Add specific files names that you do not want to appear in the listing.
// By default, the directory listing script skips the current (.) and parent 
// folder (..) links and ignores "hidden" folders starting with a period (.).
$exclude_filenames = [
  'error_log', 
  'Thumbs.db', 
  '.DS_Store'
];
  
// Add specific file extensions you want to be ignored in the listing.
// By default, the directory listing script skips "hidden" files whose names
// start with a period (.).
$exclude_extensions = [
  'ini', 
  'conf', 
  'log'
];

// Whether to hide empty folders. Default is true.
// You can turn this off by changing the value to false.
$ignore_empty_folders = true;

// Whether to toggle sub folders. Default is true.
// If true, and a folder is clicked on, it will slide down the sub folder.
// NOTE: You need to load jQuery on the page to enable the toggle function.
$toggle_sub_folders = true;
  
// Whether to enable force download attribute. Default is false.
// You can set this to true to add the HTML download attribute which forces the 
// download in some browsers.
$force_download = false;

##
## End - DirLister Options 
###############################################################################

###############################################################################
## Begin - DirLister Functions
##

## ----------------------------------------------------------------------------
## Functions to make the magic happen, you better leave this section alone.
##
## Original code comes from DirectoryLister v1.11
##   (https://github.com/halgatewood/file-directory-list/)
## Written by Hal Gatewood in 2015, 
##   (https://halgatewood.com/free/file-directory-list/)
## and license under the MIT License 
##   (https://opensource.org/licenses/MIT)
##
## The source code was edited and improved by Michael Otieno. 
## You can get original code and the past versions of this script from the 
## Gatewood's site link above. Use with this notice of the original author.
## ----------------------------------------------------------------------------

// Shows the media block.
function display_block($file) {
  global $exclude_filenames, $exclude_extensions, $force_download;
  
  $fileext = get_file_ext($file);

  if (!$fileext AND is_dir($file)) $fileext = "dir";
  if (in_array($file, $exclude_filenames)) return;
  if (in_array($fileext, $exclude_extensions)) return;

  $filename = basename($file);
  $filesize = format_file_size(filesize($file));
  $filetime = date("D, M d, Y h:i A", filemtime($file));
  $downlatt = ($force_download AND $fileext != 'dir') ? "download=\"{$filename}\"" : "";

  $rtn = "<div class=\"block\">";

  if ($fileext === "dir") {
    $rtn .= "<a class=\"dir\" href=\"{$file}\" title=\"Open\">";
  } else {
    $rtn .= "<a class=\"{$fileext}\" href=\"{$file}\" {$downlatt} target=\"_blank\" title=\"Download\">";
  }

  $rtn .= "<div class=\"img {$fileext}\"></div>";
  $rtn .= "<div class=\"name\">";
  
  if ($fileext === "dir") {
    $rtn .= "<div class=\"file\">{$filename}</div>";
    $dirfcnt = count_dir_files($file);
    $dirsize = get_dir_size($file);

    if ($dirfcnt == 1) {
      $rtn .= "<div class=\"count\">1 File";
    } else {
      $rtn .= "<div class=\"count\">{$dirfcnt} Files";
    }

    $rtn .= " (<b>{$dirsize}</b>)</div>";
  } else {
    $rtn .= "<div class=\"file\">{$filename}</div>";
    $rtn .= "<div class=\"size\"><b>Size:</b> {$filesize}</div>";
    $rtn .= "<div class=\"modified\"><b>Last Modified:</b> {$filetime}</div>";
  }

  $rtn .= "</div>";
  $rtn .= " </a>";
  $rtn .= "</div>";

  return $rtn;
}

// Recursive function to build the blocks.
function build_blocks($items = [], $folder = null) {
  global $sort_by, $exclude_filenames, $exclude_extensions, $toggle_sub_folders, $ignore_empty_folders;

  $objects            = [];
  $objects['folders'] = [];
  $objects['files']   = [];

  if (empty($items)) {
    $items = scandir(__DIR__);
  }
  
  foreach ($items as $index => $item) {
    // Skip current and parent folder links.
    if (in_array($item, array('.', '..'), true)) {
      continue;
    }

    // Skip "hidden" folders and files starting with a period.
    if ('.' === $item[0] || '..' === $item[0]) {
      continue;
    }

    // Add this directory index file "index.php" to the exclusions list.
    $exclude_filenames = array_merge(['index.php'], $exclude_filenames);

    // Skip file names specified in the file name exclusions.
    if (in_array($item, $exclude_filenames, true)) { 
      continue; 
    }

    $fileext = get_file_ext($item);
    
    // Skip file extensions specified in the file extension exclusions. 
    if (in_array($fileext, $exclude_extensions, true)) { 
      continue; 
    }
  
    if ($folder && $item) {
      $item = "$folder/$item";
    }

    // Folders
    if (is_dir($item)) {
      $objects['folders'][] = $item; 
      continue;
    }
    
    // File modification date and time
    $filetime = date("U", filemtime($item));
    
    // Files
    if ($item) {
      $objects['files'][$filetime . "-" . $item] = $item;
    }
  }
  
  // Folders
  foreach ($objects['folders'] as $index => $file) {
    $subitems = (array) scandir($file);

    if ($ignore_empty_folders) {
      $has_sub_items = false;
        
      foreach ($subitems as $subitem) {
        $subfileext = get_file_ext($subitem);
          
        if ($subitem == ".." OR $subitem == ".") continue;
        if (in_array($subitem, $exclude_filenames)) continue;
        if (in_array($subfileext, $exclude_extensions)) continue;
      
        $has_sub_items = true;
        break;  
      }
      
      if ($has_sub_items) {
        echo display_block($file);
      }
    } else {
        echo display_block($file);
    }
    
    if ($toggle_sub_folders) {
      if ($subitems) {
        echo "<div class=\"sub\" data-folder=\"{$file}\">";
        build_blocks($subitems, $file);
        echo "</div>";
      }
    }
  }
  
  // Sort before loop
  switch ($sort_by) {
    // Date created/modified ascending
    case 'DA':
      ksort($objects['files']);  
      break;
        
    // Date created/modified descending
    case 'DD':
      krsort($objects['files']);  
      break;
        
    // File/folder name descending
    case 'ND':
      arsort($objects['files']);
      break;
        
    // File/folder name ascending
    case 'NA';
    default:
      natsort($objects['files']);
      break;
  }
  
  // Files
  foreach ($objects['files'] as $index => $file) {
    $fileext2 = get_file_ext($file);

    if (in_array($file, $exclude_filenames)) { 
      continue; 
    }

    if (in_array($fileext2, $exclude_extensions)) { 
      continue; 
    }

    echo display_block($file);
  }
}

// Formats a folder or file size in the appropriate file measurement unit.
function format_file_size($bytes, $precision = 2) {
  $units = array('B', 'KB', 'MB', 'GB', 'TB');
  $bytes = max($bytes, 0); 
  $pow   = floor(($bytes ? log($bytes) : 0) / log(1024)); 
  $pow   = min($pow, count($units) - 1); 
  $bytes /= (1 << (10 * $pow));
  return round($bytes, $precision) . " " . $units[$pow];
}

// Gets the directory size in bytes.
function get_dir_size($path) {
  $bytes = 0;
  $path  = realpath($path);

  if ($path !== false && !empty($path) && is_dir($path)) {
    foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($path, FilesystemIterator::SKIP_DOTS)) as $obj) {
      $bytes += $obj->getSize();
    }
  }
    
  return format_file_size($bytes);
}

// Counts the number of files in a directory.
function count_dir_files($dir) {
  $fi = new FilesystemIterator(__DIR__ . "/" . $dir, FilesystemIterator::SKIP_DOTS);
  return iterator_count($fi);
}

// Gets the file extension from a file name.
function get_file_ext($filename) {
  return substr(strrchr($filename, '.'), 1);
}

##
## End - DirLister Functions
###############################################################################

// Set the page title based on folder name, if not set above.
if(!$title) { 
  // Start each word with uppurcase `ucwords()` and replace any dashes in the 
  // title string with a space `str_replace()`
  $title = ucwords(str_replace(array("-", "_"), " ", basename(dirname(__FILE__))));
}
?>
<!DOCTYPE html>
<html encoding="UTF-8" lang="en" dir="ltr">
<head>     
  <meta charset="UTF-8" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge" />
  <meta http-equiv="Content-type" content="text/html" />
  <meta http-equiv="Content-language" content="en-US" />
  <meta name="viewport" content="width=device-width, initial-scale=1" /> 
  <meta name='referrer' content='strict-origin-when-cross-origin' />  
  <meta name="robots" content="noindex, nofollow" /> 
  <meta name="author" content="Michael Otieno" />
  <title><?=$title?></title>
  <style type="text/css">
  *,*::before,*::after{-webkit-box-sizing:border-box;-moz-box-sizing:border-box;box-sizing:border-box} @-ms-viewport{width:device-width} html{font-family:sans-serif;line-height:1.15;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;-webkit-tap-highlight-color:rgba(0,0,0,0)} body{background-repeat:repeat-x;font-family:'Open Sans',sans-serif;font-size:14px;line-height:22px;text-align:left;margin:auto;overflow-x:hidden} main{background-color:#ffffff;background-clip:border-box;border:1px solid rgba(0,0,0,0.125);border-radius:0.25rem;margin:2em auto;padding:2em;min-width:30em;max-width:60em} h1.title{font-size:160%;font-weight:normal;margin:0 0 .6em} .block{border-top:solid 1px #ece9e9;min-height:50px;clear:both;overflow:hidden} .block:first-child{border-top:0 !important} .block .img{display:block;background:url("data:image/png;base64,") left top no-repeat;margin-right:10px;height:50px;width:50px;float:left} .block .file{font-weight:bold;padding-bottom:5px} .block .data,.block .count,.block .size,.block .modified{color:#6c757d !important;font-size:13px;line-height:1.3em} .block a,.block a:not([href]):not([tabindex]){display:block;color:inherit;padding:20px;text-decoration:none;-webkit-transition:all 0.25s ease-in-out;-moz-transition:all 0.25s ease-in-out;-o-transition:all 0.25s ease-in-out;transition:all 0.25s ease-in-out} .block a:hover,.block a:not([href]):not([tabindex]):hover,.block a:focus,.block a:not([href]):not([tabindex]):focus,.block a.open,.block a:not([href]):not([tabindex]).open{color:#0d63c9;background-color:rgba(0,0,0,0.125);text-decoration:none;cursor:pointer} .sub{display:none;border-left:solid 5px #ece9e9;margin-left:20px} .sub a:hover,.sub a:not([href]):not([tabindex]):hover,.sub a:focus,.sub a:not([href]):not([tabindex]):focus,.sub a.open,.sub a:not([href]):not([tabindex]).open{background-color:#efefef} .jpg,.jpeg,.gif,.png{background-position:-50px 0 !important} .pdf{background-position:-100px 0 !important} .txt,.rtf{background-position:-150px 0 !important} .xls,.xlsx{background-position:-200px 0 !important} .ppt,.pptx{background-position:-250px 0 !important} .doc,.docx{background-position:-300px 0 !important} .zip,.rar,.tar,.gzip{background-position:-350px 0 !important} .swf{background-position:-400px 0 !important} .fla{background-position:-450px 0 !important} .mp3{background-position:-500px 0 !important} .wav{background-position:-550px 0 !important} .mp4{background-position:-600px 0 !important} .mov,.aiff,.m2v,.avi,.pict,.qif{background-position:-650px 0 !important} .wmv,.avi,.mpg{background-position:-700px 0 !important} .flv,.f2v{background-position:-750px 0 !important} .psd{background-position:-800px 0 !important} .ai{background-position:-850px 0 !important} .html,.xhtml,.dhtml,.php,.asp,.css,.js,.inc{background-position:-900px 0 !important} .dir{background-position:-950px 0 !important} body.light{color:#1f1f1f !important;background-color:#f8f9fa !important} body.dark{color:#ffffff !important;background-color:#1d1c1c !important} body.dark main{background-color:#2b2a2a} body.dark h1.title{color:#ffffff} body.dark .block{border-top:solid 1px #666} body.dark .block a:hover,body.dark .block a.open{background:#000} body.dark .block .data{color:#fff} body.dark .sub{border-left:solid 5px #0e0e0e} footer{position:fixed;bottom:0;width:100%;text-align:center;background-color:inherit} footer hr{border:0;border-top:1px solid rgba(0,0,0,0.1);margin-left:0;margin-right:0;margin-top:1rem;margin-bottom:1rem} footer p{color:inherit;margin-top:0;margin-bottom:1rem} footer a,footer a:not([href]):not([tabindex]){color:inherit;text-decoration:none} footer a:hover,footer a:not([href]):not([tabindex]):hover,footer a:focus,footer a:not([href]):not([tabindex]):focus{cursor:pointer;text-decoration:underline} @media (min-width:576px){main{max-width:540px}} @media (min-width:768px){main{max-width:720px}} @media (min-width:992px){main{max-width:960px}} @media (min-width:1200px){main{max-width:1140px}} @media only screen and (max-width:800px){body{font-size:14px;margin-left:1em;margin-right:1em} .block .data,.block .count,.block .size,.block .modified{font-size:12px;line-height:1.2em} footer{position:relative;text-align:center;padding-top:1rem;margin-bottom:1rem;width:100%}}
  </style>
</head>
<body class="<?=$theme?>">
  <main class="main">
    <h1 class="title"><?=$title?></h1>
    <?php build_blocks(); ?>
  </main>
  <br/>
  <br/>
  <footer class="footer">
    <hr/>
    <p>
      <a href="https://github.com/mikeotizels/dirlister/" target="_blank" rel="noopener noreferrer nofollow">
        DirLister v1.0.0
      </a>
    </p>
  </footer>
  <script src="/cdn/ajax/libs/jquery/3.4.0/jquery.min.js"></script>
  <script type="text/javascript">
  $(document).ready(function(){
    $("a.dir").click(function(e){
      e.preventDefault(),
      $(this).toggleClass("open"),
      $('.sub[data-folder="'+$(this).attr("href")+'"]').slideToggle()
    })
  });
  </script>
</body>
</html>